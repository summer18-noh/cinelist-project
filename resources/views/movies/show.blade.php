...
<div style="margin-bottom:20px;">
    <a href="{{ route('movies.index') }}"
       style="color:rgba(255,255,255,0.35);font-size:13px;text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
...
<div style="position:relative;height:240px;border-radius:10px;overflow:hidden;margin-bottom:22px;background:linear-gradient(135deg,#0f1535 0%,#1d1045 60%,#090915 100%);">
    ...
    @if($movie->backdrop)
        <img src="{{ asset('storage/'.$movie->backdrop) }}"
    ...
    <div style="position:relative;z-index:2;padding:32px;height:100%;display:flex;flex-direction:column;justify-content:flex-end;">
        ...
        @if($movie->is_featured)
            <div style="font-size:9px;letter-spacing:4px;color:#2979ff;margin-bottom:8px;">
                FEATURED FILM
            </div>
        @endif
    ...
    @if($movie->rating)
        <div style="position:absolute;right:32px;bottom:28px;z-index:2;text-align:right;">
            ...
            <div style="font-family:'Barlow Condensed',sans-serif;font-size:50px;font-weight:900;color:#fff;line-height:1;">
                ...
                <span style="font-size:16px;color:rgba(255,255,255,0.3);">/10</span>
            </div>
        </div>
    @endif
    ...
    <div>
        ...
        @if($movie->poster)
            <img src="{{ asset('storage/'.$movie->poster) }}"
                 style="width:100%;border-radius:8px;border:1px solid rgba(255,255,255,0.08);"
                 alt="{{ $movie->title }}">
        @else
        ...
        <div style="display:grid;grid-template-columns:1fr 0.5px 1fr;gap:0;">
            ...
            <div style="padding-right:22px;">
                ...
                <div style="margin-bottom:13px;">
                    <div style="font-size:10px;letter-spacing:1.5px;color:rgba(255,255,255,0.22);margin-bottom:4px;">
                        DIRECTOR
                    </div>
                ...
                @if($movie->cast)
                    <div>
                        <div style="font-size:10px;letter-spacing:1.5px;color:rgba(255,255,255,0.22);margin-bottom:6px;">
                            CAST
                        </div>
            ...
            <div style="padding-left:22px;">
                ...
                @if($movie->release_year)
                    <div style="margin-bottom:13px;">
                        <div style="font-size:10px;letter-spacing:1.5px;color:rgba(255,255,255,0.22);margin-bottom:3px;">
                            RELEASE YEAR
                        </div>
                        <div style="font-size:13px;color:rgba(255,255,255,0.72);">
                            {{ $movie->release_year }}
                    ...
                    <div>
                        ...
                        <div style="display:flex;gap:5px;flex-wrap:wrap;">
                            @foreach(explode(',', $movie->genre) as $g)
                                <span class="genre-pill" style="font-size:11px;padding:3px 10px;">
                                    {{ trim($g) }}
                                </span>
                            @endforeach
                        </div>
