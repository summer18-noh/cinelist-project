...
<style>
    ...
    .hero-bg {
        position:absolute;
        inset:0;
        background:linear-gradient(135deg,#0f1535 0%,#1d1045 60%,#090915 100%);
    }
...
{{-- HERO --}}
@php
    $featured = $movies->firstWhere('is_featured', true) ?? $movies->first();
@endphp
...
<div class="hero-section">
    ...
    <div class="hero-bg">
        @if($featured->backdrop)
            <img src="{{ asset('storage/'.$featured->backdrop) }}"
                 style="width:100%;height:100%;object-fit:cover;opacity:.25;position:absolute;inset:0;">
        @endif
    </div>
    ...
    <div class="hero-content">
        <div class="hero-label">
            {{ $featured->is_featured ? 'FEATURED FILM' : 'LATEST ADDITION' }}
        </div>

        <div class="hero-title">
            {{ Str::limit($featured->title, 28) }}
        </div>

        <div class="hero-badges">
            @if($featured->release_year)
                <span class="hero-badge blue">{{ $featured->release_year }}</span>
            @endif
            ...
            @if($featured->duration)
                <span class="hero-badge">{{ $featured->duration }}</span>
            @endif
        </div>
    ...
    @if($featured->rating)
        <div class="hero-rating">
            <div style="color:#2979ff;font-size:12px;margin-bottom:2px;">
                <i class="bi bi-star-fill"></i>
            </div>

            <div class="rating-big">
                {{ number_format($featured->rating,1) }}<span>/10</span>
            </div>
        </div>
    @endif
...
<div class="dark-card">
    ...
    <table class="dark-table">
        ...
        <tbody>
            ...
            <tr>
                <td style="color:rgba(255,255,255,0.2);font-size:12px;">
                    {{ $movies->firstItem() + $i }}
                </td>
    ...
    <div style="padding:12px 20px;border-top:1px solid rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:space-between;">
        ...
        <div style="display:flex;gap:4px;">
            @if($movies->onFirstPage())
                <span class="btn-icon" style="opacity:.3;cursor:default;">
                    <i class="bi bi-chevron-left"></i>
                </span>
            @else
                <a href="{{ $movies->previousPageUrl() }}" class="btn-icon">
                    <i class="bi bi-chevron-left"></i>
                </a>
            @endif
            ...
            @if($movies->hasMorePages())
                <a href="{{ $movies->nextPageUrl() }}" class="btn-icon">
    ...
    @else
        <div style="padding:60px;text-align:center;">
            <i class="bi bi-film"
               style="font-size:40px;color:rgba(255,255,255,0.08);display:block;margin-bottom:16px;"></i>

            <div style="font-size:14px;color:rgba(255,255,255,0.3);margin-bottom:16px;">
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <div class="modal-content"
             style="background:#0d0d14;border:1px solid rgba(255,255,255,0.08);border-radius:12px;">

            <div class="modal-header"
                 style="border-bottom:1px solid rgba(255,255,255,0.06);padding:18px 22px;">
                ...
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <h5 class="modal-title"
                        style="font-family:'Barlow Condensed',sans-serif;font-size:18px;font-weight:700;color:#fff;margin:0;letter-spacing:.5px;">
                        Delete Movie
                    </h5>
                </div>
            ...
            <div class="modal-footer"
                 style="border-top:1px solid rgba(255,255,255,0.06);padding:16px 22px;gap:8px;justify-content:flex-end;">
